...
describe("BStackDemo Tests Module B", () => {
  ...
  it("module B - always failing test - missing element 1", async () => {
  ...
  it("module B - always failing test - same stacktrace 1", async () => {
  ...
  it("module B - always failing test - same stacktrace 2", async () => {
    const nonExistent = await $(`~common-error`);
    await nonExistent.waitForDisplayed({ timeout: 3000 });
    await nonExistent.click(); // Will fail
  });
  ...
  it("module B - always passing test - example A", async () => {
  ...
  it("module B - Another Test with framework-level retry - 2 retries configured", function () {
    this.retries(2); // Framework-level retry
    const randomOutcome = Math.random() > 0.7;
    if (!randomOutcome) {
      throw new Error("Test failed, retrying...");
